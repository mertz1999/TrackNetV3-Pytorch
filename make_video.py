...
def predict(index):
    # Third image channel
    cap.set(cv2.CAP_PROP_POS_FRAMES, index+2);_, image_3 = cap.read()

    # Secound image channel
    cap.set(cv2.CAP_PROP_POS_FRAMES, index+1);_, image_2 = cap.read()

    # First image channel
    cap.set(cv2.CAP_PROP_POS_FRAMES, index);_, image_1 = cap.read()
...
VIDEO_PATH = "./games/real/6_1.mp4"
WIDTH      = 512
HEIGHT     = 288
MODEL_PATH = 'models/last_model (23).pt'
...
while(frame_idx <= total_frame-2):
    ...
    cv2.circle(image_3, (cx, cy), 3, (0,0,255), -1)
